...
var TagBuilder = function () {
  function TagBuilder() {
    _classCallCheck(this, TagBuilder);
  }

  _createClass(TagBuilder, [{
    key: "Generate",
    value: function Generate(texts) {
      var _this = this;

      var tags = void 0;
      if (texts instanceof Array) {
        tags = texts.map(function (text) {
          return _this.GenerateTagsFromText(text);
        });
      } else {
...
var GenerateTagsFromText = function GenerateTagsFromText(text) {
  return _(text.split(/[\s\r?\n|\r]+/)).map(RemovePunctuationAndNewLine).filter(MoreThan(3)).map(ToLower).groupBy(function (word) {
    return word;
  }).map(GroupToTag).value();
};

var GenerateTagsFromTagsArrays = function GenerateTagsFromTagsArrays(tags) {
  return _(tags).flatten().groupBy(TagName).map(GetReducedTagFromGroup).filter(ByCountMoreThan(0)).sort(ByTagName).value();
};

var RemovePunctuationAndNewLine = function RemovePunctuationAndNewLine(str) {
...
var ToLower = function ToLower(t) {
  return t.toLowerCase();
};
var GroupToTag = function GroupToTag(group) {
  return {
    word: group[0],
    amount: group.length
  };
...
var ToTagAmount = function ToTagAmount(tag) {
  return tag.amount;
};
var GetReducedTagFromGroup = function GetReducedTagFromGroup(group) {
  return {
    name: group[0].word,
    amount: group.map(ToTagAmount).reduce(function (acc, curr) {
      return acc + curr;
    })
  };
};
var ByCountMoreThan = function ByCountMoreThan(num) {
  return function (tag) {
    return tag.amount > num;
  };
};
var ByTagName = function ByTagName(a, b) {
...
var TagBuilder = __webpack_require__(1);
